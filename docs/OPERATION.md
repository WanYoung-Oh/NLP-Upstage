...
3. [학습 (Training)](#3-학습-training)
    - 3-9. [Topic Prefix 학습 (방법 A)](#3-9-topic-prefix-학습-방법-a-use_topictrue) (신규)
4. [추론 (Inference)](#4-추론-inference)
    - 4-5. [SOLAR QLoRA 추론 및 평가](#4-5-solar-qlora-추론-및-평가-causal-lm) (신규)
5. [평가 (Evaluation)](#5-평가-evaluation)
6. [앙상블 (Ensemble)](#6-앙상블-ensemble)
7. [체크포인트 관리](#7-체크포인트-관리)
    - 7-5. [앙상블용 체크포인트 선택 유틸](#7-5-앙상블용-체크포인트-선택-유틸) (신규)
...
```
data/
├── train.csv   # 12,457건 (fname, dialogue, summary, topic)
├── dev.csv     #    499건 (fname, dialogue, summary, topic)
└── test.csv    #    499건 (fname, dialogue — summary/topic 없음)
```
...
│       ├── mbr.yaml
│       ├── tta.yaml
│       ├── causal_lm.yaml         # SOLAR QLoRA 전용 추론 (신규)
│       ├── solar_api.yaml
...
| 방법 | 방식 | 외부 의존 |
|------|------|----------|
| `back_translation` | 발화 단위로 ko→en→ko 역번역 | Google Translate (인터넷) |
| `eda` | dialogue에서 단어 무작위 삭제 | 없음 (완전 오프라인) |

**증강 전 자동 전처리**: `run_augment.py`는 train.csv 로드 직후 `clean_text` → `filter_by_length`를 먼저 적용하고 증강을 시작합니다. 학습 파이프라인(train.py)과 동일한 기준으로 정제된 데이터를 증강 입력으로 사용하여 증강 품질을 보장합니다.

**speaker token 보호**: `#Person1#`, `#Person2#` 등 화자 토큰은 증강 대상에서 제외됩니다. EDA와 back_translation 모두 화자 토큰을 기준으로 발화 내용을 분리한 뒤 내용 부분에만 증강을 적용하고 재조합합니다.

**컬럼 보존**: `fname`·`dialogue` 외의 `summary`·`topic` 등 모든 원본 컬럼은 변경 없이 유지됩니다. `fname`에는 `_aug` 접미사가 추가됩니다.
...
# 옵션 조정
python src/data/run_augment.py --method all \
    --max_samples 1000        \  # 증강할 최대 샘플 수: EDA·back_translation 각각 1,000개 (--method all 시 최대 2,000개)
    --rouge_threshold 0.4     \  # ROUGE-L 유사도 최솟값 — 이 값 미만 샘플은 제거 (기본: 0.3)
...
> **주의**: `run_augment.py`는 증강 전 `clean_text()`를 적용하므로 `data_aug/train.csv`의 dialogue에는 개행문자(`\n`)가 공백으로 치환되어 저장됩니다. 반드시 `data.use_cleaning=true`(기본값)로 학습해야 원본 데이터와 동일한 전처리 기준이 유지됩니다.
...
# topic prefix 활성화
python src/train.py data.use_topic=true

# topic + 클리닝 조합
python src/train.py data.use_topic=true data.use_cleaning=true
```
...
- `use_topic`: train/dev의 `topic` 컬럼을 encoder 입력 앞에 prepend (자세한 내용은 아래 3-9절 참고)
...
- **체크포인트**: `checkpoints/{yymmdd_run_NNN}/epoch{##}_{score:.4f}/`
  - score 상위 3개만 유지 (`BestCheckpointCallback`)
  - **seq2seq 모델**: score = `eval_rouge_combined` → 예: `epoch06_0.7498`
  - **causal_lm (SOLAR QLoRA)**: score = `1 / (1 + eval_loss)` 변환 → 예: `epoch01_0.4476`
    - loss를 역변환해 "높을수록 좋음" 형식으로 통일, `_find_best_checkpoint` 및 앙상블 유틸과 호환
- **Hydra 로그**: `outputs/{date}/{time}/` (`.hydra/config.yaml` 포함)
- **WandB**: entity/project는 `.env`에서 읽어 자동 업로드

### 3-9. Topic Prefix 학습 (방법 A, `use_topic=true`)

`data.use_topic=true`로 활성화합니다. train/dev의 `topic` 컬럼을 encoder 입력에 prepend하여 요약 모델이 대화 주제를 참고하도록 합니다.

**입력 형식 변화**:

| 모델 | use_topic=false (기존) | use_topic=true |
|------|----------------------|----------------|
| seq2seq (BART/T5) | `#Person1#: ...` | `[TOPIC] 음식 주문 [SEP]\n#Person1#: ...` |
| causal_lm (SOLAR) | `[INST] 다음 대화를...` | `[INST] 주제: 음식 주문\n다음 대화를...` |

**학습 시 topic masking (도메인 불일치 완화)**:
- 학습 데이터의 **25% 확률**로 topic을 `[MASK]`로 대체합니다.
- test.csv에는 topic이 없으므로 추론 시 항상 `[MASK]`가 사용됩니다.
- 학습에서 masking을 섞어 주면 모델이 topic 없이도 작동하도록 regularization됩니다.

```bash
# topic prefix 활성화
python src/train.py data.use_topic=true

# 추론 시 별도 설정 불필요 — use_topic=true이면 자동으로 [MASK] 적용
python src/inference.py data.use_topic=true inference.ckt_path=checkpoints/...
```

**검증(dev) 시**: topic masking 없이 실제 topic을 사용하므로 topic의 순수 기여도 측정 가능.

> **기대 효과**: 문헌 기준 ROUGE +0.5 ~ +1.5. dev ROUGE가 현재 대비 +0.3 이상이면 채택 권장 (RESEARCH.md §13 참고).
...
# use_topic=true로 학습한 모델 추론 — [MASK] prefix 자동 적용 (별도 설정 필요)
python src/inference.py data.use_topic=true inference.ckt_path=checkpoints/...
```

> **use_topic 주의**: `data.use_topic=true`로 학습한 체크포인트는 반드시 추론 시에도 `data.use_topic=true`를 함께 지정해야 합니다. test.csv에 topic 컬럼이 없으므로 자동으로 `[TOPIC] [MASK] [SEP]\n` prefix가 적용됩니다.
...
### 4-5. SOLAR QLoRA 추론 및 평가 (Causal LM)

`model=solar_qlora`로 학습한 체크포인트는 `inference=causal_lm`을 사용합니다. `Seq2SeqInferencer` 대신 `CausalLMInferencer`가 자동으로 선택됩니다.

```bash
# 평가 (dev.csv → ROUGE 점수 출력)
python src/inference.py \
  model=solar_qlora \
  inference=causal_lm \
  inference.ckt_path=checkpoints/260316_run_001/epoch03_0.5297 \
  inference.eval_split=dev \
  inference.output_filename=eval_260316_run_001.csv
...
python src/inference.py \
  ...
  inference=causal_lm \
  inference.ckt_path=checkpoints/260316_run_001/epoch03_0.5297 \
  inference.eval_split=test \
  inference.output_filename=output_260316_run_001.csv
```

`conf/inference/causal_lm.yaml` 기본 설정:

```yaml
num_beams: 4
no_repeat_ngram_size: 3
length_penalty: 1.2
generate_max_length: 150   # max_new_tokens 기준 (프롬프트 제외)
batch_size: 4              # 10.7B 모델 메모리 고려
eval_split: "test"         # "dev"로 변경하면 ROUGE 평가 모드
```

- **`eval_split=dev`**: dev.csv로 추론 후 ROUGE-1/2/L 및 Combined 점수를 콘솔 출력 + CSV 저장
- **`eval_split=test`**: test.csv로 추론 후 `prediction/` 에 CSV 저장
- batch 생성은 **left-padding** 자동 적용
- 생성 토큰만 디코딩 (프롬프트 길이만큼 슬라이싱), `skip_special_tokens=True`

> **메모리 부족 시** `inference.batch_size=1`로 낮추세요.
...
### 6-4. 앙상블용 체크포인트 선택 유틸 (`list_checkpoints`, `select_checkpoints_for_ensemble`)

체크포인트 디렉토리를 스캔해 앙상블 대상 경로를 유연하게 선택합니다.

```python
from src.ensemble import list_checkpoints, select_checkpoints_for_ensemble

# 전체 체크포인트 목록 조회 (score 내림차순)
ckpts = list_checkpoints("checkpoints")
# [{"run_id": "260314_run_005", "epoch": 7, "score": 0.8011, "path": "..."},  ...]

# 특정 run만 조회
ckpts = list_checkpoints("checkpoints", run_id="260314_run_005")

# run별 best 1개씩 수집 → 다중 모델 앙상블
paths = select_checkpoints_for_ensemble("checkpoints", top_k_per_run=1)

# 특정 run들의 best 체크포인트만 선택
paths = select_checkpoints_for_ensemble(
    "checkpoints",
    run_ids=["260314_run_005", "260314_run_003", "260316_run_003"],
)

# 단일 run에서 상위 3개 체크포인트 앙상블 (같은 모델 다중 checkpoint)
paths = select_checkpoints_for_ensemble(
    "checkpoints",
...
python src/ensemble_cli.py merge \
  ...
  --output ensemble/ensemble_response_only_out3.csv \
  --weights 0.34 0.36 0.3
...
```
checkpoints/
├── 260313_run_001/              # yymmdd_run_NNN (seq2seq)
│   ├── epoch04_0.6895/
│   ├── epoch05_0.7198/
│   └── epoch06_0.7498/          # eval_rouge_combined 상위 3개만 유지
└── 260316_run_003/              # yymmdd_run_NNN (causal_lm / SOLAR QLoRA)
    ├── epoch01_0.4476/
    └── epoch02_0.4832/          # 1/(1+eval_loss) 변환 후 상위 3개 유지
```

두 경우 모두 `epoch{##}_{score:.4f}` 형식으로 통일되며, score는 항상 "높을수록 좋음"입니다.
...
### 7-5. 앙상블용 체크포인트 선택 유틸

체크포인트 목록 조회 및 앙상블 대상 선택은 `src/ensemble.py`의 유틸 함수를 사용합니다.
자세한 사용법은 [6-4절](#6-4-앙상블용-체크포인트-선택-유틸-list_checkpoints-select_checkpoints_for_ensemble)을 참고하세요.
...
for c in list_checkpoints("checkpoints"):
    print(c["run_id"], c["epoch"], c["score"], c["path"])

# run별 best 1개씩 앙상블용 경로 선택
paths = select_checkpoints_for_ensemble("checkpoints", top_k_per_run=1)
```
...
data:
  ...
  use_topic: false           # true: topic을 encoder 입력에 prepend (방법 A)
                             #   학습 25% masking / 검증 0% / 추론 항상 [MASK]
...
| `causal_lm.yaml` | SOLAR QLoRA 추론/평가 | `batch_size=4`, `eval_split=test\|dev` |
...
# topic prefix 활성화
python src/train.py model=solar_qlora training=qlora data.use_topic=true
...
- **체크포인트 score 변환**: `BestCheckpointCallback`이 `eval_loss → 1/(1+loss)` 로 변환해 `epoch{##}_{score:.4f}` 형식으로 저장합니다. seq2seq의 ROUGE combined score와 동일한 "높을수록 좋음" 형식으로 통일되므로, `list_checkpoints` / `select_checkpoints_for_ensemble` 등 앙상블 유틸을 모델 구분 없이 사용할 수 있습니다.
...
python src/train.py data.use_topic=true                          # topic prefix 활성화 (방법 A)
python src/train.py data.use_topic=true data.use_cleaning=true   # topic + 클리닝 조합
...
python src/inference.py model=solar_qlora inference=causal_lm \  # SOLAR QLoRA 평가
  inference.ckt_path=checkpoints/... inference.eval_split=dev
python src/inference.py model=solar_qlora inference=causal_lm \  # SOLAR QLoRA 추론
  inference.ckt_path=checkpoints/... inference.eval_split=test
...
python -c "from src.ensemble import list_checkpoints; [print(c) for c in list_checkpoints('checkpoints')]"

# ── 앙상블용 체크포인트 선택 ────────────────────────────────────
# run별 best 1개씩
python -c "from src.ensemble import select_checkpoints_for_ensemble; print(select_checkpoints_for_ensemble('checkpoints', top_k_per_run=1))"
# 특정 run 지정
python -c "from src.ensemble import select_checkpoints_for_ensemble; print(select_checkpoints_for_ensemble('checkpoints', run_ids=['260314_run_005','260316_run_003']))"
```
