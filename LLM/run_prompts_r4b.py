"""
r4b_traindev LoRA 전용 프롬프트 앙상블 실행 스크립트
...
실행:
    python run_prompts_r4b.py [--resume] [--adapter ...] [--test-csv ...]
"""
...
# ── 기본 경로 (response_only_SFT 데이터 + r4b_traindev LoRA) ──
SCRIPT_DIR = Path(__file__).resolve().parent
_DEFAULT_ADAPTER = SCRIPT_DIR / "response_only_SFT/outputs/r4b_traindev_ckpt/lora_adapter"
_DEFAULT_TEST_CSV = SCRIPT_DIR / "response_only_SFT/data/test.csv"
_DEFAULT_PRED_DIR = Path("/data/ephemeral/home/prediction")
...
def load_model_and_tokenizer(adapter_path: Path):
    ...
    base_model, _ = FastModel.from_pretrained(
        ...
        attn_implementation="sdpa",  # flex_attention → sdpa (CUDA index OOB 방지)
    )
...
def output_csv_path(pred_dir: Path, variant: str, timestamp: str) -> Path:
    return pred_dir / f"r4b_{variant}_{timestamp}.csv"


def mbr_csv_path(pred_dir: Path, timestamp: str) -> Path:
    return pred_dir / f"r4b_mbr_{timestamp}.csv"
...
def main(args: argparse.Namespace) -> None:
    ...
    # tokenizer_config.json의 model_max_length(40960)가 generation_config로 전파되어
    # flex_attention 블록마스크 생성 시 RoPE 캐시(MAX_SEQ_LENGTH) 범위를 초과하는 문제 방지
    ...
    resume_dir = pred_dir / f"r4b_variants_{timestamp}"
    if args.resume:
        # 가장 최근 variants 폴더 탐색
        existing = sorted(pred_dir.glob("r4b_variants_*"), reverse=True)
        if existing:
    ...
    for variant in PROMPT_VARIANTS:
        ...
        sub_path = output_csv_path(pred_dir, variant, timestamp)
    ...
    mbr_path = mbr_csv_path(pred_dir, timestamp)
    ...
    log("전체 완료. 생성된 파일:")
    for variant in PROMPT_VARIANTS:
        log(f"  {output_csv_path(pred_dir, variant, timestamp)}")
    log(f"  {mbr_path}  ← 최종 MBR 서브미션")


if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="r4b_traindev LoRA + response_only_SFT/data 테스트 CSV 프롬프트 앙상블"
    )
    p.add_argument(
        "--adapter",
        type=str,
        default=str(_DEFAULT_ADAPTER),
        help=f"LoRA 어댑터 디렉터리 (기본: {_DEFAULT_ADAPTER})",
    )
    p.add_argument(
        "--test-csv",
        type=str,
        default=str(_DEFAULT_TEST_CSV),
        help=f"테스트 CSV 경로 (기본: {_DEFAULT_TEST_CSV})",
    )
    p.add_argument(
        "--pred-dir",
        type=str,
        default=str(_DEFAULT_PRED_DIR),
        help=f"예측 CSV 저장 디렉터리 (기본: {_DEFAULT_PRED_DIR})",
    )
    p.add_argument(
        "--resume",
        action="store_true",
