...
사용법:
# Dev로 효과 검증
    python src/inference_solar_refine.py \
        --config conf/config_solar.yaml \
...
class KoBARTInference:
    ...
    토크나이저는 `general.model_name`(허브)에서, 가중치는 `inference.ckt_path`에서 로드합니다.
    `src/inference.py`와 동일 패턴 — 체크포인트 폴더에 tokenizer 파일이 없어도 동작합니다.
    `general.model_name`이 없으면 예전처럼 `ckt_path`에서만 로드합니다.
    """
    ...
    def __init__(self, config: dict):
        ...
        tokenizer_src = general.get("model_name")

        if tokenizer_src:
            print(f"[KoBART] 토크나이저 로드: {tokenizer_src}")
            self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_src)
        else:
            print(f"[KoBART] 토크나이저 로드: {ckt_path} (general.model_name 없음)")
            self.tokenizer = AutoTokenizer.from_pretrained(ckt_path)
        ...
        print(f"[KoBART] 가중치 로드: {ckt_path}")
        self.model = AutoModelForSeq2SeqLM.from_pretrained(ckt_path).to(self.device)
